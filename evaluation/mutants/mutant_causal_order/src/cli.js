...
import { MutantLedgerManager } from './ledger.js';
...
async function mutantAppendCommand(args = [], options = {}) {
  ...
  const ledger = new MutantLedgerManager(storage);
  const appendedRecord = await withLock(storage.lockFile, 'append', async () => {
